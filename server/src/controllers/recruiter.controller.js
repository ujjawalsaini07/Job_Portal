...
export const createCompanyProfile = async (req, res) => {
  try {
    ...
    const existingProfile = await RecruiterProfile.findOne({ userId: req.user.id });
...
export const getVerificationStatus = async (req, res) => {
  try {
    const profile = await RecruiterProfile.findOne({ userId: req.user.id }).select(
      "isVerified verificationStatus verificationNotes"
    );
